 ...
 * キャッシュを利用してパース結果を取得
 */
function getParsedTree(document) {
 ...
 * - 属性値（値部分と属性全体）
 */
function collectPairs(node, pairs, text) {
    var _a;
    if (node.tag !== "root") {
        ...
        if (openTagMatch) {
            ...
            while ((m = attrRegex.exec(openTagText)) !== null) {
                ...
                const attrValue = (_a = m[3]) !== null && _a !== void 0 ? _a : m[4];
...
let cursorOffsetCacheHtml = []; // HTML用キャッシュ
    ...
    const newSelections = selections.map((selection, idx) => {
        ...
        for (let i = 0; i < containingPairs.length; i++) {
            ...
            if (pair.type === "attrValue") {
                // 値 → 属性全体
                if (currentRange.start === pair.open &&
                    currentRange.end === pair.close) {
                    const attrPair = containingPairs.find((p) => p.type === "attr" && p.open <= pair.open && p.close >= pair.close);
                    if (attrPair)
                ...
                if (currentRange.start === pair.open &&
                    currentRange.end === pair.close) {
                    const tagPair = containingPairs.find((p) => p.type === "openTag" &&
                        p.open <= pair.open &&
                        p.close >= pair.close);
                    if (tagPair)
                        nextRange = { start: tagPair.open, end: tagPair.close };
                }
                else {
                    nextRange = { start: pair.open, end: pair.close };
                }
                ...
                // 開始タグ → タグ全体
                const tagPair = containingPairs.find((p) => !["attr", "attrValue", "openTag"].includes(p.type));
                if (tagPair)
                ...
                if (openGt !== -1 && closeLt !== -1 && openGt + 1 <= closeLt) {
                    ...
                    if (currentRange.start === innerText.start &&
                        currentRange.end === innerText.end) {
                        nextRange = outerTag;
                    }
                    else if (currentRange.start === outerTag.start &&
