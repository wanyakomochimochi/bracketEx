...
function surroundWithInput() {
  ...
  const charBefore =
    position.character > 0 ? text.charAt(document.offsetAt(position) - 1) : "";
  const charAfter =
    position.character < text.length
      ? text.charAt(document.offsetAt(position))
      : "";
  const brackets = ["()", "[]", "{}", '""', "''"];
  let toRemove = "";
  for (const pair of brackets) {
    if (charBefore === pair[1] && charAfter === pair[0]) {
      toRemove = pair;
      break;
    }
  }
  if (toRemove) {
    ...
    const end = document.positionAt(document.offsetAt(position) + 1);
